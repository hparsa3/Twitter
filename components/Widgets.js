...
export default function Widgets({ newsResults, randomUsersResults }) {
  ...
  return (
    <div className="xl:w-[600px] hidden lg:inline ml-8 space-y-5">
      ...
      <div className=" sticky text-gray-700 space-y-3 bg-gray-100 pt-2 rounded-xl w-[90%] xl:w-[75%]">
        <h4 className="font-bold text-xl px-4">Who to Follow</h4>
        <AnimatePresence>
        {randomUsersResults.slice(randomUserNum).map((randomUser) => (
          <motion.div key={randomUser.login.username} intial={{opacity: 0}} 
          animate={{opacity: 1}} exit={{opacity: 0}}
          transition={{duration: 1}}>
          <div key={randomUser.login.username} className="flex items-center px-4 py-2 cursor-pointer hover:bg-gray-200 transition duration-500 ease-out">
            ...
            <button className="ml-auto bg-black text-white rounded-full text-sm px-3.5 py-2.5 font-bold">Follow</button>
          </div>
          </motion.div>
        ))}
        </AnimatePresence>
        <button onClick={() => setRandomUserNum(randomUserNum + 3)} className="text-blue-300 pl-4 pb-3 hoverEffect:text-blue-400">Show more</button>
      </div>
    </div>
  );
}
